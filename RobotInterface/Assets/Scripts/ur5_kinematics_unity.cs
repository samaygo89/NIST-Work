...
//  Revision:        1.1 10/15/2018
//  Author:          Shelly Bagchi, Esteban Segarra
//
//  Description
//  ===========
//  Inverse Kinematics programming for virtual UR5.
//
...
public class ur5_kinematics_unity : MonoBehaviour
{
    ...
    public float[] angle_vector = new float[6];  // used for holding last angle set
    ...
    public class ur5
    {
        ...
        public ur5()
        {
            // a:  vertical  offset between joints (e.g. length of arm segments in m)
            //a.SetValues(new float[] { 0, -0.425f, -0.39225f, 0, 0, 0 });
            a.SetValues(new float[] { -0.080f, -0.425f, -0.392f, 0, 0, 0 });
            // a:  horizontal  offset between joints (m)
            ...
            for (int i = 0; i < c; i++)
            {
                ...
                { // More checks
                    ...
                    if (z > 0.1 && d < 0.1)
                    {
                        ...
                        {
                            Debug.Log("Did not add vector (joint limits OOB):" + theta.SubMatrix(0, r, i, 1));
                        }

                    }
                    else
                        Debug.Log("Did not add vector (zcoord OOB):" + theta.SubMatrix(0, r, i, 1));
                }
            }
